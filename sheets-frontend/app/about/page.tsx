...
import { Target, Heart, Shield, Check, Linkedin, Menu, X, Instagram } from 'lucide-react';
...
function HeroSection() {
  return (
    <section className="pt-24 sm:pt-28 md:pt-32 px-4 sm:px-6">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-4">
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-slate-900 mb-4 sm:mb-6">About Lernova Attendsheets</h1>
  ...
  const mvvItems = [
    {
      icon: Target,
      title: 'Our Mission',
      description: 'To empower educators with tools that simplify attendance management, allowing them to focus on teaching and student support.',
      color: 'emerald'
    },
    {
      icon: Heart,
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 mb-12 sm:mb-16 md:mb-20 place-items-center">
          {mvvItems.map((item, idx) => {
            ...
            return (
              <div key={idx} className={`bg-white rounded-2xl p-6 sm:p-8 shadow-md border ${colors.border} text-center hover:shadow-xl transition-all`}>
                <div className={`w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-br ${colors.bg} rounded-xl flex items-center justify-center mx-auto mb-4 sm:mb-5 shadow-sm`}>
...
function WhyChooseUsSection() {
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6 bg-white/50 backdrop-blur">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-8 sm:mb-10 md:mb-12 text-center">Why Choose Us?</h2>

        <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
          {reasons.map((reason, idx) => (
            <div key={idx} className="flex items-start gap-3 sm:gap-4">
              <div className={`w-10 h-10 sm:w-12 sm:h-12 ${colorMap[reason.color as keyof typeof colorMap]} rounded-xl flex items-center justify-center shrink-0 mt-1`}>
                <Check className={`w-5 h-5 sm:w-6 sm:h-6 ${reason.color === 'emerald' ? 'text-emerald-600' : reason.color === 'teal' ? 'text-teal-600' : 'text-cyan-600'}`} />
              </div>
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-1 sm:mb-2">{reason.title}</h3>
  ...
  return (
    <section className="py-12 sm:py-16 md:py-20 lg:py-24 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto text-center">
        <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-6 sm:mb-8">Built by Passionate Students</h2>
        <p className="text-base sm:text-lg md:text-xl text-slate-600 mb-8 sm:mb-10 md:mb-12 leading-relaxed">
          Lernova Attendsheets, a product of Lernova, is created by students at Atharva University who understand the challenges educators face. We're committed to building tools that make a real difference in education.
        </p>

        <div className="bg-gradient-to-br from-cyan-50 to-cyan-100 rounded-2xl sm:rounded-3xl p-6 sm:p-8 md:p-10 border border-cyan-200">
          <div className="flex items-center justify-center gap-2 mb-4 sm:mb-6">
            <img src="/clogo.png" alt="Lernova Logo" className="w-12 h-12 sm:w-16 sm:h-16" />
            <h3 className="text-xl sm:text-2xl font-bold text-slate-900">The Lernova Team</h3>
          </div>
          <div className="space-y-2 sm:space-y-3 text-slate-600">
            <p className="font-semibold text-sm sm:text-base">Muhammad Nabeel</p>
            <p className="font-semibold text-sm sm:text-base">Rituraj Thakur</p>
            <p className="font-semibold text-sm sm:text-base">Archita Parab</p>
            <p className="font-semibold text-sm sm:text-base">Shweta Yadav</p>
            <p className="font-semibold text-sm sm:text-base">Aksh Devlapalli</p>
            <p className="font-semibold text-sm sm:text-base">Mohnish Pembarthi</p>
          </div>
          <p className="text-xs sm:text-sm text-slate-500 mt-4 sm:mt-6">Students at Atharva University, Mumbai</p>
        </div>
      </div>
    </section>
  );
